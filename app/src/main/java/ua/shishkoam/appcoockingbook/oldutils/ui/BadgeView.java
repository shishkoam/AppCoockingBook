...
public class BadgeView extends TextView {
    ...
    private View target;
    private int targetInitialPaddingLeft;
    private int targetInitialPaddingRight;
    ...
    private void init(Context context, View target, int tabIndex) {
        ...
        this.target = target;
        targetInitialPaddingLeft = target.getPaddingLeft();
        targetInitialPaddingRight = target.getPaddingRight();
        ...
        setPadding(paddingPixels, 0, paddingPixels + 1, 0);
    ...
    private void applyLayoutParams() {
        ...
        switch (badgePosition) {
            ...
            case POSITION_LEFT:
                ...
                lp.setMargins(targetInitialPaddingLeft, 0, badgeMarginH, 0);
                break;
            case POSITION_RIGHT:
                lp.gravity = Gravity.RIGHT | Gravity.CENTER;
                lp.setMargins(badgeMarginH, 0, targetInitialPaddingRight, 0);
                break;
            default:
                break;
        }
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        setTargetPadding(getVisibility());
    }

    @Override
    protected void onVisibilityChanged(View changedView, int visibility) {
        setTargetPadding(visibility);
        super.onVisibilityChanged(changedView, visibility);
    }

    private void setTargetPadding(int visibility) {
        if (target != null && (badgePosition == POSITION_LEFT || badgePosition == POSITION_RIGHT)) {
            measure(MeasureSpec.UNSPECIFIED, MeasureSpec.UNSPECIFIED);
            int width = getMeasuredWidth() + badgeMarginH;

            int left = targetInitialPaddingLeft;
            int right = targetInitialPaddingRight;
            int top = target.getPaddingTop();
            int bottom = target.getPaddingBottom();

            if (visibility == View.VISIBLE) {
                if (badgePosition == POSITION_LEFT) {
                    left += width;
                } else if (badgePosition == POSITION_RIGHT) {
                    right += width;
                }
            }
     ...
     * POSITION_BOTTOM_RIGHT, POSITION_CENTER, POSITION_LEFT, POSITION_RIGHT.
     *
     */
