package ua.shishkoam.appcoockingbook.utils.util
...
object FormatUtils {
    private const val SIZE_DIVIDER: Long = 1024
    private const val GB_DIVIDER = SIZE_DIVIDER * SIZE_DIVIDER * SIZE_DIVIDER
    private const val MB_DIVIDER = SIZE_DIVIDER * SIZE_DIVIDER
    private const val KB_DIVIDER = SIZE_DIVIDER
    fun getStringNodeSize(context: Context, size: Long): String {
        return getStringNodeSize(size, context)
    }

    fun getStringNodeSize(size: Long, context: Context): String {
        return when {
            size > GB_DIVIDER -> {
                getMeasuredSize(size, GB_DIVIDER) + context.getString(R.string.gigabyte)
            }
            size > MB_DIVIDER -> {
                getMeasuredSize(size, MB_DIVIDER) + context.getString(R.string.megabyte)
            }
            size > KB_DIVIDER -> {
                getMeasuredSize(size, KB_DIVIDER) + context.getString(R.string.kilobyte)
            }
            else -> size.toString() + context.getString(R.string.unit_byte)
        }
    }
