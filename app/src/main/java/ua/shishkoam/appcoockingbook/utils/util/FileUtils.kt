...
object FileUtils {
    ...
    open fun getDataFromFile(context: Context, defaultDir: File) {
        val ofd = OpenFileDialog(context)
        ofd.setFilter(".*\\.json")
        ofd.setCurrentPath(context, defaultDir.absolutePath)
        ...
        ofd.setOpenDialogListener(object : OpenFileDialog.OpenDialogListener {
            override fun onSelectedFile(fileName: String?) {
                val string = readFromFile(File(fileName))//here add callback
            }
        })
        ofd.show()
    }
